...
/** Represents a GetMeasures from the Withings feed: https://developer.health.nokia.com/api/doc#api-Measure-get_measure
  *
  * The model Measurement is persisted in the storage and it is is extracted from this response.
  *
  * { "status": 0, // Response status; 0 : Operation was successful "body": { "updatetime": 1515353923, "measuregrps": [ { "grpid":
  * 999543286, // Id of the measure group "attrib": 0, // The way the measure was attributed to the user (captured by a device, ambiguous,
  * manual, etc.) "date": 1515190616, // Date as unix epoch "category": 1, // Category for the measures in the group (1 for real
  * measurements, 2 for user objectives) "measures": [ { "value": 76567, // Value for the measure in S.I units (kilogram, meters, etc.)
  * "type": 1, // Type of the measure. "unit": -3 // Power of ten the "value" parameter should be multiplied to to get the real value. } ] }
