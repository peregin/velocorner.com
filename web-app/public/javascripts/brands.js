    ...
    function startSearch(activities, queryToSearch, activityIdToSearch) {
        ...
        if (activities) {
            ...
            var urlPath = queryToSearch ? "/api/brand/suggest?query=queryToSearch" : "/api/activities/"+activityIdToSearch
            // trigger asynchronous search
            if (queryToSearch) {
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: "/api/brand/suggest?query="+queryToSearch,
                    timeout: 20000,
                ...
                $.ajax({
                    type: "GET",
                    dataType: "json",
                    url: "/api/brand/"+activityIdToSearch,
                    timeout: 20000,
                    success: function(result) {
    ...
    $(document).ready(function() {
        ...
        if (searchField.length) {
            ...
            if ($('#brand-result') && (urlParams.has('aid') || urlParams.has('q'))) {
