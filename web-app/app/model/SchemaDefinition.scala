...
object SchemaDefinition {
   ...
   */
  val characters =
    Fetcher.caching((ctx: CharacterRepo, ids: Seq[String]) => Future.successful(ids.flatMap(id => ctx.getHuman(id) orElse ctx.getDroid(id))))(HasId(_.id))
  ...
  val Human =
    ObjectType(
      ...
      fields[CharacterRepo, Human](
        ...
        Field("appearsIn", OptionType(ListType(OptionType(EpisodeEnum))), Some("Which movies they appear in."), resolve = _.value.appearsIn map (e => Some(e))),
  ...
  val Query = ObjectType(
    ...
    fields[CharacterRepo, Unit](
      ...
      Field("droid", Droid, arguments = ID :: Nil, resolve = Projector((ctx, _) => ctx.ctx.getDroid(ctx arg ID).get))
    )
  )
