...
lazy val webApp = (project in file("web-app") withId "web-app")
  .settings(
    ...
    dockerBaseImage := DockerBuild.baseImage,
    dockerUsername := Some("peregin"),
    dockerEntrypoint := Seq(s"bin/${executableScriptName.value}", "-Duser.timezone=UTC", "-Dplay.server.pidfile.path=/dev/null"),
    Universal / javaOptions ++= Seq("-Dplay.server.pidfile.path=/dev/null", "$JAVA_OPTS"),
    Docker / version := "latest",
